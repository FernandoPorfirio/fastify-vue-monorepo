...
import { hashPassword, signToken, createRefreshToken } from '@/lib/auth'
...
export class InviteService {
  async createInvite(input: CreateInviteInput): Promise<CreateInviteResult | null> {
    // Verificar se o tenant existe e está ativo
    const tenant = await db('tenants').where({ id: input.tenantId, is_active: true }).first()
    ...
    const profile = await db('profiles').where({ id: input.profileId, is_active: true }).first()
  ...
  async acceptInvite(input: AcceptInviteInput): Promise<AcceptInviteResult | null> {
    ...
    const tenant = await db('tenants').where({ id: invite.tenant_id, is_active: true }).first()
